...
//   console.log(data);
  ...
  useEffect(() => {
    dispatch(moviecard(id)); // Dispatch an action to fetch movie details
    return () => {
      dispatch(removemovie()); // Clean up by removing the movie details when leaving the component
    };
  }, [id, dispatch]);
    ...
    <>
      {
        data ? (
          <div className="moviedetail_handleimg" style={{
            ...
            {
                data.videos ? <Link to ={`/movie/details/${data.details.id}/Trailer`}><button className="btn_trailer" >Play Video</button></Link>:
