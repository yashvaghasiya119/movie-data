...
export function Tvshow() {
    ...
    let [catagory, setcatogary] = useState("on_the_air");
    ...
    async function populerfunc() {
        try {
            ...
            let response = await da.get(`/tv/${catagory}`, {
                params: { page }
            });
    ...
    return <>
        <div className="t-page-main">
            <div className="t-page-heading">
                ...
                <span>Tv show</span>
                <Navigation />
                <Dropdown title="Shows" options={["airing_today","on_the_air", "popular", "top_rated"]} func={(e) => setcatogary(e.target.value)} />
            </div>

            {/* Infinite scroll component */}
            <InfiniteScroll
                dataLength={tvshow.length}  // Length of the current list
                next={fetchMoreData}  // Function to load more data
                hasMore={tvshow.length < 500}  // Disable scroll when no more data is available
            ...
            >
                {tvshow && <Trandingpagecards data={tvshow} title='tvshow' />}
            </InfiniteScroll>
        </div>
    </>
}